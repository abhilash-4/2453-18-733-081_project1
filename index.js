...
app.post('/searchventbynameandstatus', middleware.checkToken, (req,res)=>{
    const name=req.query.name;
    const status=req.query.status;
    console.log(name);
    console.log(status);
    const ventilatordetails=db.collection('ventilators')
    .find({'name':new RegExp(name, 'i'),"status":status}).toArray().then(result=>res.json(result));
});
...
app.post('/addventilator',(req,res)=>{
    ...
    const item={"hid":hid, "name":name, "vid":vid, "status":status};
...
app.put('/updateventidbyhidandstatus',middleware.checkToken,(req,res)=>{
    const hid={hid: req.query.hid};
    const status={status: req.query.status};
    console.log(hid);
    console.log(status);
    const newvalues={$set: {vid:req.query.vid}};
    console.log("updating ventilator details, please wait a moment");
    db.collection("ventilators").updateOne(hid, newvalues, function(err, result){
        res.json('updated one document');
        if(err) throw err;
    });
});
...
app.post('/addhospital',(req,res)=>{
    ...
    console.log("adding ventilator, please wait a moment");
    const item={"hid":hid, "name":name,"address":address};
